...
list_mod_coeff = pd.read_excel('./Modulus_and_Coefficients_combo_3.xlsx', index_col= None).iloc[:,1:].rename(columns={0: 'Modulus', 1:'Coefficients'})
list_mod_coeff

#Filter out the case with modulus 8192 and different sets of coefficients
mod_coeff_expr = list_mod_coeff.iloc[[1,3,5],:].reset_index(drop=True)
mod_coeff_expr
...
CNNPARAM = 'Train_Test_Initial_Mix.pth'
...
class EncConvNet:
    ...
    def forward(self, enc_x, windows_nb):
        ...
        # forward1 layer
        dec_x = F.relu(self.torch_nn.forward1(dec_x))
