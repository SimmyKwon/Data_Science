...
#Remove randomness of PyTorch CNN model and script by setting a seed value
...
logging.basicConfig(filename=f'{name}_logs_real.txt',
                    filemode='w',
...
list_mod_coeff = pd.read_excel('./Modulus_and_Coefficients_combo.xlsx', index_col= None).iloc[:,1:].rename(columns={0: 'Modulus', 1:'Coefficients'})
list_mod_coeff

#Filter out the case with modulus 8192 and different sets of coefficients
mod_coeff_expr = list_mod_coeff.iloc[[6,10,13],:].reset_index(drop=True)
mod_coeff_expr
...
testloader = DataLoader(testdata, batch_size=5, shuffle=False, num_workers=5, persistent_workers=True, prefetch_factor=2)
    ...
    with torch.no_grad():
        for chunk in test_loader:
            ...
            for i in range(len(inputs)):  # Loop over batch samples
                ...
                for GT, prediction in zip(torch.unsqueeze(sample_target,0), predictions):
