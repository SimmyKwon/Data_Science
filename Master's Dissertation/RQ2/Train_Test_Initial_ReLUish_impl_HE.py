...
list_mod_coeff = pd.read_excel('./Modulus_and_Coefficients_combo_2.xlsx', index_col= None).iloc[:,1:].rename(columns={0: 'Modulus', 1:'Coefficients'})
list_mod_coeff

#Filter out the case with modulus 8192 and different sets of coefficients
mod_coeff_expr = list_mod_coeff.iloc[[0,2],:].reset_index(drop=True)
mod_coeff_expr


#%%

ArrayLike = Union[torch.Tensor, np.ndarray, ts.CKKSVector]

def ReLU_approx(x = ArrayLike, coef = float) -> Union[torch.Tensor, np.ndarray, ts.CKKSVector]:
    ...
    elif isinstance(x, ts.CKKSVector):
        first_term = x ** 3
        second_term = x ** 2
        return (coef)*first_term + (1-coef)*second_term
...
CNNPARAM = 'Train_Test_Initial_ReLUish_impl.pth'
